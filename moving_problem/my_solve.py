...
def pollards_rho(G_sub, H_sub, order, a, b, p):
	"""Pollard's Rho algorithm: O(sqrt(n)) time, O(1) memory"""
	if not H_sub: return 0
	if G_sub == H_sub: return 1
	
	# Helper function to partition the curve into 3 sets and take a pseudo-random step
	def next_step(P, c, d):
		# If P is the point at infinity (empty list), just default it to subset 0
		if not P:
			subset = 0
		else:
			# We use the x-coordinate modulo 3 to split the points into 3 random sets
			subset = P[0] % 3
		
		if subset == 0:
			return add(P, G_sub, a, b, p), (c + 1) % order, d
		elif subset == 1:
			return add(P, P, a, b, p), (c * 2) % order, (d * 2) % order
		else:
			return add(P, H_sub, a, b, p), c, (d + 1) % order


	# Starting position
	T_tortoise = G_sub
	c_tortoise, d_tortoise = 1, 0
	...
	while True:
		...
		T_tortoise, c_tortoise, d_tortoise = next_step(T_tortoise, c_tortoise, d_tortoise)
		
		# Hare takes 2 steps
		T_hare, c_hare, d_hare = next_step(T_hare, c_hare, d_hare)
		T_hare, c_hare, d_hare = next_step(T_hare, c_hare, d_hare)
	...
	# c_tortoise * G + d_tortoise * H = c_hare * G + d_hare * H
	# (d_tortoise - d_hare) * H = (c_hare - c_tortoise) * G
	
	numerator = (c_hare - c_tortoise) % order
	denominator = (d_tortoise - d_hare) % order
	
	if denominator == 0:
		return None # Very rare failure, would just need to run it again with a different starting point
		
	inv_den = inverse_mod(denominator, order)
	
	return (numerator * inv_den) % order
...
def polig_hellmen_for_ECC(order, G, H, a, b, p, fact):
	...
	for i in range(len(fact)):
		...
		# Use BSGS for small factors, Pollard's Rho for the memory-heavy monster
		if fact[i] > 1000000000:
			x = pollards_rho(G_sub, H_sub, fact[i], a, b, p)
		else:
