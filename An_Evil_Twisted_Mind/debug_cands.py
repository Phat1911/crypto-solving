...
# These were recovered in my last run!
# Instead of doing everything again, let's just use the exact values from the last run to debug!

x0 = 17396435685300331104043488561493030626341259545861536497718075431092037423531231610259464626256550340114016622008332
X_out = 1542779843212822518523837700519145420389625587804221128282598512050094525827818234120272095623814750020989926133709
...
remainders_1 = [806200561, 3337604938, 5255861242]
remainders_2 = [4, 440, 1468012, 1757060, 12893370811]
...
def server_scalarmult_int(scalar, x0_val, mod):
    ...
    if bit:
        R0 = R1
    return (R0[0] * pow(R0[1], mod-2, mod)) % mod

print(f"X_out target: {X_out}")
for i, base_cand in enumerate(base_cands):
    cand_real = min(base_cand % order_limit, (order_limit - base_cand) % order_limit)
    test_out = server_scalarmult_int(cand_real, x0, modulus)
    print(f"[{i}] cand: {cand_real}")
